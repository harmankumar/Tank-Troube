      ...
      if (keyboardKey == 1)			// 1 means that key i has been pressed
      {
        if(previt[i] == false)      //// change
        {
        ...
        else if (i == 4)											//////////change
          {
            ...
            sprintf(temp2,"%d",2); // Instruction code
            tempT.push_back(temp2);
            ...
            float i = 0.011;
            sprintf(temp2,"%f",bulletShot[numBullets-1].centreBulletX);    //X cord
            tempT.push_back(temp2);
            i = -0.011;
            sprintf(temp2,"%f",bulletShot[numBullets-1].centreBulletY);    //Y cord
            tempT.push_back(temp2);
            i = 0.011;
            sprintf(temp2,"%f",bulletShot[numBullets-1].angleBullet);    //Angle
            tempT.push_back(temp2);
            i = 0.011;
            ...
            lastCrucialMsgs.push_back(tmp);
            toSend.message=attachCrucial(toSend.message);
            if(lastCrucialMsgs.size()>numOfCrucialMsg)
            {
                lastCrucialMsgs.erase(lastCrucialMsgs.begin());
                cout<<"Size of lastCrucial Message:"<<lastCrucialMsgs.size()<<endl;
            }

            for(int iter=0;iter<numOfUsers;iter++)
            {
              cout<<"\nNow sending bullet shot message"<<endl;
              cout<<makeMessage(tempT)<<endl;
              if(iter!=myUserId )//&& userActive[iter])
              {
                cout<<"User:"<<iter<<endl;
                toSend.destPort=userPort[iter];        //rightnow for only one tank
...
bool Tank::destroyTank()
{
	...
	for (int i = 0; i < bulletShot.size()&& i<temp;i++)
	{
      ...
      //			cout<<"Alas the tank has been hit\n";
            ...
            sprintf(temp2,"%d",3); // Instruction code
            tempT.push_back(temp2);

            sprintf(temp2,"%d",mostRecentMsgNum[myUserId]++); // Serial Code
            tempT.push_back(temp2);

            std::vector<float> pos =this->getTankPosition();
            sprintf(temp2,"%f",pos[0]); // XCord of tank
            tempT.push_back(temp2);

            sprintf(temp2,"%f",pos[1]); // YCord of tank
            tempT.push_back(temp2);
            cout<<"Bullet Id"<<i<<endl;
            sprintf(temp2,"%d",i); // Bullet  Id of the bullet that shot the tank
            tempT.push_back(temp2);

            sendQueueItem toSend;
            toSend.isAck=false;
            toSend.message=makeMessage(tempT);
            cout<<"Message "<<toSend.message<<endl;
            // Crucial Messagestring tmp;
            string tmp;
            tmp=toSend.message;
            tmp=tmp.substr(1,tmp.size()-2);
            lastCrucialMsgs.push_back(tmp);
            ...
            for(int iter=0;iter<numOfUsers;iter++)
            {
              if(iter!=myUserId) // && userActive[i])
              {
                cout<<"Tank destroyed message to user:"<<iter<<endl;
